...
//! # Lockstep warning
//!
//! Everything in this module mirrors `KeyRotationRegistry.sol`
...
/// `manifest.toml` entry in seismic-reth.
...
/// Reads a schedule from the execution database before any block changes.
/// Load the account first: revm's `State` requires this before storage access.
pub fn read_schedule<DB: alloy_evm::Database>(db: &mut DB) -> Result<RotationSchedule, String> {
    db.basic(KEY_ROTATION_REGISTRY)
        .map_err(|err| format!("registry account read failed: {err:?}"))?;
    read_schedule_with(|slot| {
        db.storage(KEY_ROTATION_REGISTRY, U256::from_be_bytes(slot.0))
